...
technologies, Odoo reads the content and automatically creates and fills in the records. This
process is mainly used for vendor bills (or refunds).

.. note::
   Although less common, this digitization process can also be applied to customer invoices/credit
   notes. :ref:`Settings <accounting/bill-digitization/configuration>` need to be adjusted
   accordingly.
...
Vendor bills can be uploaded via email alias associated with the relevant journal in two ways:
...
Once the document has been digitized, a blue banner :guilabel:`All fields will be automated by
Artificial Intelligence, it might take 5 seconds` appears; click :icon:`oi-arrow-right`
:guilabel:`Refresh` if needed. Review and correct any information uploaded during digitization:
click on the related field(s) to edit and make updates, or click :guilabel:`Reload AI data` to
refresh the data.

Then, click :guilabel:`Confirm` to post the document.

.. tip::
   Once a document has been digitized, the :guilabel:`Vendor` field remains empty if the vendor is
   not already in the database. To add it, click the :icon:`fa-caret-down` :guilabel:`(down arrow)`
...
.. note::
   The following vendor bill fields are read by OCR:
   ...
   - :guilabel:`Vendor`, :guilabel:`Bill Reference`, :guilabel:`Bill Date`, :guilabel:`Payment
     ...
     :guilabel:`Due Date`, and the currency (in a multi-currency environment and if the currency is
     activated).
   - From the :guilabel:`Invoices Lines` tab: :guilabel:`Product` description/label,
     :guilabel:`Quantity`, unit :guilabel:`Price`, :guilabel:`Taxes` (if the tax is activated -
     except India localization), :guilabel:`Untaxed Amount`, and :guilabel:`Total`.
...
When a digitized vendor bill is read by OCR, Odoo first searches into the database for a matching
purchase order and its number. If found, it pulls in as much information as possible from the
purchase order to the vendor bill. For example, the vendor :guilabel:`Bill Reference` number is
overwritten by the :guilabel:`Vendor Reference` from the purchase order.
...
.. note::
   ...
   - XML files don't require OCR credits because they contain structured code Odoo can read
     directly, without AI processing.

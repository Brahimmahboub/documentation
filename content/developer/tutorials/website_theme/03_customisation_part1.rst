...
#. In order for the client to find the mega menu you previously created for him, turn it into a
   template that can be find through the website builder.
...
.. note::
   Ensure that you properly call every templates in your header (like shopping cart, language
   selector, call to action...), so everything will remain editable via the website builder and all
...
3. Place your new header over the content of your homepage.

.. seealso::
   See reference documentation on :ref:`website_themes/pages/theme_pages/header_overlay`.

.. spoiler:: Solutions

   Add the right field on the home page:
...
#. | Create the snippet template. Then, add it to the list of building blocks available in the
     website builder. Place it in its own category.
   | Here you will find the `images
...
#. Add 2 options in the Website Builder for the bubbles:
